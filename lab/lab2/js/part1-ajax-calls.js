...
map = CreateLeafletObjects();

var AddToMap = function(phillySolar){
    L.marker([phillySolar.Y,phillySolar.X]).bindPopup("DEVELOPER: "+phillySolar.DEVELOPER).addTo(map);
};

$.ajax(phillySolarInstallationDataUrl).done(function(ajaxResponseValue){
  var computedValue = ajaxResponseValue;
  //console.log(computedValue);
  var solars = JSON.parse(computedValue);
  //console.log(solars);
  _.each(solars,AddToMap);
});
